...
public class ItemController {
    ...
    public ItemDto createItem(@RequestBody @Validated(ItemDto.Create.class) ItemDto itemDto,
                              @RequestHeader(AppConstants.USER_ID) Long ownerId) {
    ...
    public ItemDto getItemById(@PathVariable Long id,
                               @RequestHeader(AppConstants.USER_ID) Long requesterId) {
        log.info("GET /items/{} requested by user {}", id, requesterId);
        return itemService.getById(id, requesterId);
    }

    @GetMapping
    public List<ItemDto> getItemsByOwner(@RequestHeader(AppConstants.USER_ID) Long ownerId) {
    ...
    public CommentDto addComment(@PathVariable Long itemId,
                                 @RequestHeader(AppConstants.USER_ID) Long authorId,
